...
class _ProgressScreenState extends State<ProgressScreen> {
  ...
  Future<void> _loadData() async {
    ...
    final classLevel =
        int.tryParse(classStr.toString()) ?? 9;

    int total = 0;
    int completed = 0;
    for (final s in _subjects) {
      final t = await _repo.getTotalCount(
          s['name'], classLevel);
      final c =
          await _repo.getCompletedCount(s['name']);
      total += t;
      completed += c;
    }
  ...
  double get _overallProgress => _totalLessons == 0
      ? 0
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title:
            const Text('My Progress / ನನ್ನ ಪ್ರಗತಿ'),
      ),
      body: _isLoading
          ? const Center(
...
class _SubjectProgress extends StatefulWidget {
  ...
  @override
  State<_SubjectProgress> createState() =>
      _SubjectProgressState();
}

class _SubjectProgressState
    extends State<_SubjectProgress> {
  int _completed = 0;
  int _total = 0;
  ...
  Future<void> _load() async {
    final completed = await widget.repo
        .getCompletedCount(widget.subject);
          ...
          Expanded(
            child: Column(
              ...
              children: [
                  ...
                  builder: (context, snapshot) {
                    ...
                    return Column(
                      ...
                      children: [
                        Text(
                          lesson != null
                              ? '${lesson.subject} — Part ${lesson.part}'
                ...
                const SizedBox(height: 4),
                // Wrap instead of Row: when the result-label chip is
                // long ("Needs Improvement"), the score text drops to
                // its own line instead of overflowing horizontally.
                Wrap(
                  spacing: 8,
                  runSpacing: 4,
                  crossAxisAlignment:
